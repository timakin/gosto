...
func init() {
	...
	// 2) Be in a package that does not import gosto, nor import anything that imports gosto
	// 3) Executes before gosto's init()
	...
	//       * The order in which the package is presented to the compiler gets moved after gosto
...
func serializeStruct(src interface{}) ([]byte, error) {
	...
	if k != reflect.Struct {
		return nil, fmt.Errorf("gosto: Expected struct, got instead: %v", k)
	}
...
func serializeStructInternalEncode(enc *gob.Encoder, smd *structMetaData, fieldName, metaData string, encodeValue bool, v reflect.Value) error {
	...
	if encodeValue {
		if err := enc.EncodeValue(v); err != nil {
			return fmt.Errorf("gosto: Failed to encode field %v value %v - %v", fieldName, v.Interface(), err)
		}
	}
	return nil
}
...
func deserializeStruct(dst interface{}, b []byte) error {
	if len(b) == 0 {
		return fmt.Errorf("gosto: Expected some data to deserialize, got none.")
	}
	...
	if k != reflect.Struct {
		return fmt.Errorf("gosto: Expected struct, got instead: %v", k)
	}

	if header := b[0]; header == serializationStateEmpty {
		return datastore.ErrNoSuchEntity
	} else if header != serializationStateNormal {
		return fmt.Errorf("gosto: Unrecognized cache header: %v", header)
	}
		...
		if elemType.Kind() == reflect.Uint8 {
			if !zeroValue {
				if err := dec.DecodeValue(vf); err != nil {
					return fmt.Errorf("gosto: Failed to decode field %v - %v", fieldName, err)
				}
			}
		} else {
...
func (g *Gosto) getStructKey(src interface{}) (key *datastore.Key, hasStringId bool, err error) {
	...
	if k != reflect.Struct {
		err = fmt.Errorf("gosto: Expected struct, got instead: %v", k)
		return
	}
	...
	for i := 0; i < v.NumField(); i++ {
		...
		if len(tagValues) > 0 {
			...
			if tagValue == "id" {
				...
				case reflect.Int64:
					if intID != 0 || stringID != "" {
						err = fmt.Errorf("gosto: Only one field may be marked id")
						return
					}
					intID = vf.Int()
				case reflect.String:
					...
					hasStringId = true
				default:
					err = fmt.Errorf("gosto: ID field must be int64 or string in %v", t.Name())
					return
				}
			} else if tagValue == "kind" {
				if vf.Kind() == reflect.String {
					if kind != "" {
						err = fmt.Errorf("gosto: Only one field may be marked kind")
						return
					}
					kind = vf.String()
				...
				if vf.Type().ConvertibleTo(dskeyType) {
					if parent != nil {
						err = fmt.Errorf("gosto: Only one field may be marked parent")
						return
					}
...
func (g *Gosto) setStructKey(src interface{}, key *datastore.Key) error {
	...
	if k != reflect.Ptr {
		return fmt.Errorf("gosto: Expected pointer to struct, got instead: %v", k)
	}
	...
	if k != reflect.Struct {
		return fmt.Errorf(fmt.Sprintf("gosto: Expected struct, got instead: %v", k))
	}
	...
	for i := 0; i < v.NumField(); i++ {
		...
		if len(tagValues) > 0 {
			...
			if tagValue == "id" {
				if idSet {
					return fmt.Errorf("gosto: Only one field may be marked id")
				}
				...
				if parentSet {
					return fmt.Errorf("gosto: Only one field may be marked parent")
				}
				dskeyType := reflect.TypeOf(&datastore.Key{})
				vfType := vf.Type()
	...
	if !idSet {
		return fmt.Errorf("gosto: Could not set id field")
	}
